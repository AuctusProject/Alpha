...
export class MetamaskAccountService {
  ...
  private monitoreAccount() {
    ...
    var accountInterval = setInterval(function() {
      ...
      self.web3Service.getAccount().subscribe(account => {
        if (!self.isRinkeby()) {
  ...
  private checkLoginConditions() {
    let self = this;
    this.checkAccountAndNetwork().subscribe(success => {
      if (success) {
  ...
  private checkAUCBalance(account) {
    ...
    if (account) {
      this.web3Service
        .getTokenBalance(environment.aucTokenContract.address, account)
        .subscribe(balance => {
  ...
  public sendAUC(value: number): Observable<string> {
    var aucToSendHex = this.web3Service.toHex(
      this.web3Service.toWei(value.toString())
    );
    var data = this.web3Service.getContractMethodData(
      environment.aucTokenContract.abi,
      environment.aucTokenContract.address,
      "transfer",
      environment.escrowContract.address,
      aucToSendHex
    );
